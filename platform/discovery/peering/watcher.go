...
type NetworkWatcher struct {
	...
	segments                  kclient.Client[*soloapi.Segment]
...
func New(
	...
	c.segments = kclient.NewFilteredDelayed[*soloapi.Segment](
		client,
		gvr.Segment,
	...
	if federatedWaypoints := sd.FederatedWaypoints(); federatedWaypoints != nil {
		c.remoteWaypointSync = federatedWaypoints.RegisterBatch(func(events []krt.Event[krt.Named]) {
			for _, e := range events {
				o := e.Latest()
				c.commonServiceHandler(types.NamespacedName{
					Name:      o.Name,
	...
	c.namespaces.AddEventHandler(controllers.EventHandler[controllers.Object]{
		AddFunc: func(obj controllers.Object) {
			...
			for _, service := range services {
				...
				c.commonServiceHandler(config.NamespacedName(service))
			}
		},
	...
	c.services.AddEventHandler(controllers.FilteredObjectHandler(func(o controllers.Object) {
		// many things will look up the local copy of a service, so trigger them in the commonServiceHandler
		name := config.NamespacedName(o)
		c.commonServiceHandler(name)
	...
	c.workloadEntries.AddEventHandler(controllers.FilteredObjectHandler(func(o controllers.Object) {
		// WorkloadEntry (specifically, the reconcileGatewayWorkloadEntry output) act as the primary input to ServiceEntry
	...
	c.workloadEntries.AddEventHandler(controllers.EventHandler[controllers.Object]{
		UpdateFunc: func(oldObj, newObj controllers.Object) {
			...
			// TODO but what if they changed one of the important labels?
			c.workloadEntryChangedHandler(oldObj)
		},
		DeleteFunc: c.workloadEntryChangedHandler,
	})
	c.serviceEntries.AddEventHandler(controllers.EventHandler[controllers.Object]{
		UpdateFunc: func(oldObj, newObj controllers.Object) {
			// use oldObj just in case the labels were modified
			// TODO but what if they changed one of the important labels?
			c.serviceEntryChangedHandler(oldObj)
		},
		DeleteFunc: c.serviceEntryChangedHandler,
	})
	...
	updateLocalSegment := func(segmentName string, segmentResource *soloapi.Segment) (string, string, bool) {
		...
		if oldSegmentName == newSegmentName && oldSegmentDomain == newSegmentDomain {
			return oldSegmentName, oldSegmentName, false
		}
		...
		// segment changed, need to reprocess services, status
		return newSegmentName, oldSegmentName, true
	}
	...
	c.namespaces.AddEventHandler(controllers.ObjectHandler(func(o controllers.Object) {
		...
		func() {
			...
			newSegment, oldSegment, segmentChange = updateLocalSegment(o.GetLabels()[SegmentLabel], segmentResource)
		}()
		...
		if segmentChange {
			...
			c.enqueueSegmentStatusUpdate(newSegment)
			if oldSegment != newSegment {
				c.enqueueSegmentStatusUpdate(oldSegment)
			}
		}
	}))
	...
	c.segments.AddEventHandler(controllers.ObjectHandler(func(o controllers.Object) {
		...
		func() {
			...
			if segmentResource != nil {
				log.Infof("stevenctl: segment resource event in %s: %s %s", c.localCluster, o.GetName(), segmentResource.Spec.Domain)
			}

			// Detect whether the segment used by this local cluster changed.
			if ns := c.namespaces.Get(systemNamespace, ""); ns != nil {
				localSegmentLabel := ns.GetLabels()[SegmentLabel]
				if localSegmentLabel == o.GetName() {
					_, _, localSegmentChanged = updateLocalSegment(o.GetName(), segmentResource)
				}
			} else {
		...
		if localSegmentChanged {
			log.Info("segment resource used by the local cluster changed, reprocessing local services")
			reprocessLocalServices()
		}

		// the domain changing on the local copy of any Segment may make peered clusters valid/invalid
		c.enqueueSegmentStatusUpdate(o.GetName())
	}))
...
// GetPeerSegmentInfo returns the segment info received from a peer cluster, or nil if not available
func (c *NetworkWatcher) GetPeerSegmentInfo(cid cluster.ID) *workloadapi.Segment {
	pc := c.getClusterByID(cid)
	if pc == nil {
		return nil
	}
	return pc.GetSegmentInfo()
}
...
func (c *NetworkWatcher) remoteServiceHandler(obj RemoteFederatedService) {
...
func (c *NetworkWatcher) remoteWorkloadHandler(obj RemoteWorkload) {
...
func (c *NetworkWatcher) workloadEntryChangedHandler(o controllers.Object) {
...
func (c *NetworkWatcher) serviceEntryChangedHandler(o controllers.Object) {
...
func (c *NetworkWatcher) commonServiceHandler(name types.NamespacedName) {
	c.queue.Add(typedNamespace{
		NamespacedName: name,
		kind:           ServiceEntry,
	})
	for _, remoteService := range c.workloadEntryServiceIndex.Lookup(name) {
		c.workloadEntryChangedHandler(remoteService)
	}
	...
	c.mu.RLock()
	defer c.mu.RUnlock()
	for _, pc := range c.remoteClusters {
		// Check if this remote cluster has a federated service for this service
		fsKey := fmt.Sprintf("%s/%s/%s", pc.clusterID, name.Namespace, name.Name)
		if fs := pc.federatedServices.GetKey(fsKey); fs != nil {
			c.remoteServiceHandler(*fs)
		}
		host := AutogenHostname(name.Name, name.Namespace, pc.GetSegmentInfo().GetDomain())
		for _, wl := range pc.workloadsByService.Lookup(host) {
			c.remoteWorkloadHandler(wl)
		}
	}
}
